...
def do_algo(name, ds):
	...
	for n_exp in xrange(100):
		...
		try:
			...
			accuracy = avg(current.results)
			which = "%d->%d" % (last_n, n)
			extra = (last_accuracy, accuracy, name)
			allresults[which].append(extra)
#			print which, extra
...
def parse_allresults():
	for ds in processdata.get_datasets():
		for m in all_method_names:
			print ds.description(), m
			do_algo(m, ds)

def save_allresults():
	print allresults
	with open('results/accuracy-stuff.pickle', 'w') as f:
		cPickle.dump(allresults, f)

def load_allresults():
	global allresults
	with open('results/accuracy-stuff.pickle') as f:
...
if __name__ == '__main__':
	...
	for k in sorted(allresults.keys(), key=lambda x:(len(x), x)):
		filtered = [(fix(x[0]), fix(x[1])) for x in allresults[k]]
#		avg=median
		...
		p_new_fraction = avg((new-old)/new*100 if new else 0 for old, new in filtered)
